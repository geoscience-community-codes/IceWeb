...
for subfig=1:4
    ...
    if (fin==-1)
        ...
        if max(counts) > 20
            maxcounts = max(counts(end-ndays:end)) + 5;
        else
            maxcounts = 20; 
        end;
        ...
        %plot_histogram(dnum,counts,ndays,mincounts,maxcounts,tstr{subfig},ystr{subfig});
        bar(dnum,counts);
        enum = floor(now);
	...
	counts(counts < 0) = 0;
        ...
        plot(dnum,[ccum - ccum(end-ndays)],'r','LineWidth',1);
...
function plot_histogram(dnum,counts,ndays,mincounts,maxcounts,tstr,ystr);
bar(dnum,counts);
enum = floor(now);
axis([enum-ndays enum mincounts maxcounts]);
DateTickLabel('x');
