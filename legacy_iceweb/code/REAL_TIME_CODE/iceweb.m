...
% 25 Jul, 2004: added "try" condition around the trload_cssgrp function so that if the 
%		data is not available in the database, it won't just crash the process,
%		it'll skip until the next station.
...
%cludged first volcano sent has problem
%send twice 20040707 Matlab R13 and Antelope 4.6 prob
for volcano_num2=0:num_volcanoes
	if volcano_num2 > 0
		volcano_num = volcano_num2;
	else	
		volcano_num = 1;
	end	
	% which volcano?
...
% If we don't find the data in the database, don't crash, just skip that station...
try
