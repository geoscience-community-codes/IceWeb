...
use iceweb_perl_utilities qw(read_volcanoes read_stations read_windstation read_drplots read_thresholds read_use read_allowed_drplots);
$ICEWEB="/home/iceweb";
...
sub volcano_setup_form {
	...
	print "<BODY><FORM ACTION=\"http://www.avo.alaska.edu/cgi-bin/iceweb/volcano_setup.pl\" METHOD=\"POST\">\n";
	print "<hr>\n";
	print "<FONT SIZE=7> <center> IceWeb Setup for <INPUT TYPE=\"text\" NAME=\"Volcano\" VALUE=\"$volcano\" SIZE=\"10\"> Volcano </center></FONT><P>\n";
	...
	To remove a station from the alarm (e.g. because it is noisy) turn it off or enter a large value
	for threshold (e.g. 99)<p>\n";
	...
	unless ($#stations>=5) {
		for ($extra_station_num=$#stations+1;$extra_station_num<=5;$extra_station_num++) {
			...
			$use[$extra_station_num]=0;
		}
	};
	
	# display current station / alarm setup info for this volcano in a table
	print "<TABLE>\n";
	print "<TR><TD><BR></TD><TH>Stations</TH><TH>Threshold<BR>(cm^2)</TH><TH>Use in alarm?</TH></TR>\n";		
	...
	@windstations=qw(nowind cbwind dhwind gkwind howind ilwind kswind);
	@real_names=qw(None Cold_Bay Dutch_Harbour Gulkana Homer Iliamna King_Salmon);
