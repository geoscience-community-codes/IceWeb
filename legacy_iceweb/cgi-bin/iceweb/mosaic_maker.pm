...
will always be highlighted.</font><p>\n";
				...
				# Warning if current data is missing
				# Check to see if current data was requested
				# If time_diff exceeds ~30 minutes send a warning
				if ( ($ehours==0) && ($time_diff>0.3) ) {
					print $fh "<TABLE ALIGN=\"CENTER\" WIDTH=50%><TR><TD ALIGN=\"CENTER\" BGCOLOR=\"RED\">\n";
					print $fh "<B>WARNING: Most current data is from $time_diff hours ago</B>\n";
