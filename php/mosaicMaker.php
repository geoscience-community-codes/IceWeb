...
<?php
	...
	# Set date/time now
	# GT 9/3/15 replacing Antelope now() with php time()
	#$timenow = now(); # EPOCH TIME ON WEBSERVER 
	...
	$timenow = time(); # EPOCH TIME ON WEBSERVER
	$currentYear = date('Y', $timenow);
	$currentMonth = date('m', $timenow);
	$currentDay = date('d', $timenow);
	$currentHour = date('H', $timenow);
	$currentMinute = date('i', $timenow);
	$currentSec = date('s', $timenow);
	...
	$numMins = !isset($_REQUEST['numMins'])? 60 : $_REQUEST['numMins'];
		...
		$timestart = $timenow - $starthour * 3600; 
		...
		# GT 9/3/15 replacing Antelope str2epoch with PhP datetime		
		#$startepoch = str2epoch("$year/$month/$day $hour:$minute:00");
        	$startepoch = strtotime("$year-$month-$day $hour:$minute:00");
        	$starthour = (($timenow - $startepoch) / 3600);
        	$endhour   = (($timenow - $startepoch) / 3600) - $numhours;
